  ...
  // Performance: Only update coordinates on mouse move, defer DOM updates to rAF
  document.addEventListener('mousemove', e => { mx = e.clientX; my = e.clientY; }, { passive: true });
  function animateCursor() {
    rx += (mx - rx) * 0.12;
    ry += (my - ry) * 0.12;
    // Performance: Use translate3d to trigger GPU acceleration and consolidated DOM updates
  ...
  animateCursor();
  // Performance: Use event delegation for better memory efficiency and support for dynamic content
  document.addEventListener('mouseover', e => {
    if (e.target.closest('a, button, [onclick]')) ring?.classList.add('hovered');
  });
  document.addEventListener('mouseout', e => {
    const target = e.target.closest('a, button, [onclick]');
    const related = e.relatedTarget?.closest?.('a, button, [onclick]');
