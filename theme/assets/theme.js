  ...
  function animateCursor() {
    ...
    rx += (mx - rx) * 0.15;
    ry += (my - ry) * 0.15;
    // Direct coordinates for dot
    dx += (mx - dx) * 0.45;
    dy += (my - dy) * 0.45;

    // Performance: Only update DOM if there's a meaningful change (dirty check)
    const delta = Math.abs(mx - dx) + Math.abs(my - dy) + Math.abs(mx - rx) + Math.abs(my - ry);
    if (delta > 0.1) {
