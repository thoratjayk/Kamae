  ...
  let dx = 0, dy = 0; // Dot positions (interpolation)
  let rx = 0, ry = 0; // Ring positions (interpolation)
  let lastDx = 0, lastDy = 0, lastRx = 0, lastRy = 0; // Previous rendered positions
  ...
  function animateCursor() {
    ...
    // Dot follows faster (0.35 lerp), Ring follows slower (0.12 lerp)
    dx += (mx - dx) * 0.35;
    dy += (my - dy) * 0.35;
    rx += (mx - rx) * 0.12;
    ry += (my - ry) * 0.12;

    // Optimization: Dirty checking - only update DOM if position changed > 0.1px
    const movedDot = Math.abs(dx - lastDx) > 0.1 || Math.abs(dy - lastDy) > 0.1;
  ...
  // Optimization: Document-level event delegation for cursor hover states.
  // This is more performant and automatically supports dynamically added elements (like products).
  document.addEventListener('mouseover', e => {
    if (e.target.closest('a, button, .product-add-btn, [onclick]')) {
      ring.classList.add('hovered');
    }
  }, { passive: true });

  document.addEventListener('mouseout', e => {
    if (!e.relatedTarget || !e.relatedTarget.closest || !e.relatedTarget.closest('a, button, .product-add-btn, [onclick]')) {
