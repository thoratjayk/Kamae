(function() {
  ...
  // Optimized cursor: use passive listener and consolidate updates in rAF
  ...
  function animateCursor() {
    ...
    // Use translate3d for GPU acceleration and batch updates to avoid layout thrashing
    if (dot) dot.style.transform = `translate3d(calc(${mx}px - 50%), calc(${my}px - 50%), 0)`;
    if (ring) ring.style.transform = `translate3d(calc(${rx}px - 50%), calc(${ry}px - 50%), 0)`;
  ...
  document.addEventListener('mouseover', e => {
    ...
    } else if (e.target.id === 'cursorDot' || e.target.id === 'cursorRing') {
      // Ignore cursor elements themselves to prevent them from interfering with hover detection
      return;
    }
  });
  document.addEventListener('mouseout', e => {
    const target = e.target.closest('a, button, .product-add-btn, [onclick]');
    if (target) {
      // Check if we are moving to a child of the same interactive element
      if (!e.relatedTarget || (!target.contains(e.relatedTarget) && e.relatedTarget.id !== 'cursorDot' && e.relatedTarget.id !== 'cursorRing')) {
