  ...
  // Use passive listener for high-frequency mousemove to improve scroll performance
  ...
  function animateCursor() {
    // Dot follows mouse directly; Ring follows with easing.
    // Both are updated in a single rAF loop to prevent layout thrashing.
    dx = mx; dy = my;
    rx += (mx - rx) * 0.12;
    ry += (my - ry) * 0.12;

    // Performance: Dirty check threshold (0.1px) avoids redundant DOM writes when stationary
    const dotMoved = Math.abs(dx - lastDx) > 0.1 || Math.abs(dy - lastDy) > 0.1;
  ...
  // Performance: Event delegation avoids attaching hundreds of individual listeners to interactive elements
