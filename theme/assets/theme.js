  ...
  function animateCursor() {
    ...
    // Dirty check: only update DOM if movement is significant (> 0.1px)
    if (Math.abs(mx - dx) > 0.1 || Math.abs(my - dy) > 0.1 || Math.abs(mx - rx) > 0.1 || Math.abs(my - ry) > 0.1) {
      // Hardware-accelerated translate3d with centering
  ...
  // Use document-level event delegation for hover states (better performance, handles dynamic elements)
