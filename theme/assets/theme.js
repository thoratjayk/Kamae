  ...
  // Bolt: Using passive listener for performance, only updating coordinates
  document.addEventListener('mousemove', e => {
    mx = e.clientX;
    my = e.clientY;
  }, { passive: true });

  // Bolt: Consolidating all DOM updates into a single rAF loop and using translate3d for GPU acceleration
  ...
  // Bolt: Using event delegation on document for flicker-free hover detection and better performance with dynamic content
  document.addEventListener('mouseover', e => {
    const target = e.target.closest('a, button, [onclick], .qty-btn');
    if (target !== currentHoverTarget) {
      currentHoverTarget = target;
      if (ring) ring.classList.toggle('hovered', !!target);
    }
  }, { passive: true });
