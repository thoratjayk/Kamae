  ...
  // Consolidate DOM updates into a single requestAnimationFrame loop
  function animateCursor() {
    // Smooth follow for the ring
    rx += (mx - rx) * 0.12;
    ry += (my - ry) * 0.12;

    // Update both elements in the same frame to prevent layout thrashing
  ...
  // Use event delegation for cursor hover states to improve memory efficiency and support dynamic elements
  ...
  document.addEventListener('mouseout', e => {
    const target = e.target.closest('a, button, [onclick], .product-card, .btn');
    const related = e.relatedTarget ? e.relatedTarget.closest('a, button, [onclick], .product-card, .btn') : null;
