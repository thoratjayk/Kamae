  ...
  function animateCursor() {
    // Smoother interpolation: dot follows faster (0.35), ring follows with more lag (0.12)
    dx += (mx - dx) * 0.35;
    dy += (my - dy) * 0.35;
    rx += (mx - rx) * 0.12;
    ry += (my - ry) * 0.12;

    // Performance: Only update DOM if position changed significantly (> 0.1px)
    // and use translate3d for hardware acceleration
    const changed = Math.abs(dx - lastDx) > 0.1 || Math.abs(dy - lastDy) > 0.1 ||
                    Math.abs(rx - lastRx) > 0.1 || Math.abs(ry - lastRy) > 0.1;
  ...
  // Optimization: Use event delegation for cursor hover states to reduce memory overhead
  // and automatically support dynamically added elements (like product cards)
