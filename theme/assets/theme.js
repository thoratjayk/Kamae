  ...
  function animateCursor() {
    // Smooth interpolation: 0.35 for dot (snappy), 0.12 for ring (trailing)
    dx += (mx - dx) * 0.35;
    dy += (my - dy) * 0.35;
    rx += (mx - rx) * 0.12;
    ry += (my - ry) * 0.12;
    ...
    const movedDot = Math.abs(dx - ldx) > 0.1 || Math.abs(dy - ldy) > 0.1;
    const movedRing = Math.abs(rx - lrx) > 0.1 || Math.abs(ry - lry) > 0.1;
  ...
  // Event delegation: more efficient than individual listeners on every element
