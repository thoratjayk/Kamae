  ...
  function animateCursor() {
    // ⚡ Bolt Optimization: Consolidate updates and use dirty-checking to skip redundant DOM writes
    const ddx = mx - dx;
    const ddy = my - dy;
    const drx = mx - rx;
    const dry = my - ry;

    // Only update if there's significant movement (0.1px threshold)
    const moved = Math.abs(ddx) > 0.1 || Math.abs(ddy) > 0.1 || Math.abs(drx) > 0.1 || Math.abs(dry) > 0.1;
  ...
  // ⚡ Bolt Optimization: Use event delegation for better performance and dynamic element support
