  ...
  let ldx = 0, ldy = 0, lrx = 0, lry = 0; // Last rendered positions
  ...
  function animateCursor() {
    // Smooth interpolation (lerp) for both dot (0.35) and ring (0.12)
    dx += (mx - dx) * 0.35;
    dy += (my - dy) * 0.35;
    rx += (mx - rx) * 0.12;
    ry += (my - ry) * 0.12;

    // 0.1px dirty-check threshold to skip redundant style updates
    const dDiff = Math.abs(dx - ldx) + Math.abs(dy - ldy);
    const rDiff = Math.abs(rx - lrx) + Math.abs(ry - lry);
  ...
  // Event delegation for hover states to support dynamic content and reduce listener overhead
