  ...
  // Only update coordinates, defer DOM updates to rAF loop for better performance
  document.addEventListener('mousemove', e => { mx = e.clientX; my = e.clientY; }, { passive: true });
  let dx = 0, dy = 0;
  function animateCursor() {
    // Smoother interpolation for ring and dot
    const ddx = (mx - dx) * 0.35;
    const ddy = (my - dy) * 0.35;
    const drx = (mx - rx) * 0.12;
    const dry = (my - ry) * 0.12;
    ...
    // Only update DOM if there's meaningful movement (dirty check)
  ...
  animateCursor();
  // Single delegated listener for hover states - more efficient and handles dynamic content
  document.addEventListener('mouseover', e => {
    if (e.target.closest('a, button, [onclick], .product-add-btn')) ring.classList.add('hovered');
  }, { passive: true });
  document.addEventListener('mouseout', e => {
    if (e.target.closest('a, button, [onclick], .product-add-btn')) ring.classList.remove('hovered');
  }, { passive: true });
