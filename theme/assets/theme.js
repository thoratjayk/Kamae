  ...
  // Passive listener only updates coordinates to avoid blocking main thread
  document.addEventListener('mousemove', e => { mx = e.clientX; my = e.clientY; }, { passive: true });

  function animateCursor() {
    // Smoothed coordinates for ring
    rx += (mx - rx) * 0.15;
    ry += (my - ry) * 0.15;

    // Direct coordinates for dot (with slight smoothing for consistency)
    dx += (mx - dx) * 0.8;
    dy += (my - dy) * 0.8;

    // Use translate3d for GPU acceleration and consolidate DOM updates
    // Only update if movement exceeds a tiny threshold to save cycles
