  ...
  let ldx = 0, ldy = 0, lrx = 0, lry = 0; // Last positions for dirty checking
  ...
  function animateCursor() {
    ...
    // Dirty check for performance: skip style updates if change is sub-pixel (0.1px threshold)
    const dotDirty = Math.abs(dx - ldx) > 0.1 || Math.abs(dy - ldy) > 0.1;
    const ringDirty = Math.abs(rx - lrx) > 0.1 || Math.abs(ry - lry) > 0.1;
  ...
  // Use event delegation for hover states - more efficient than individual listeners
  document.addEventListener('mouseover', e => {
    const target = e.target.closest('a, button, .product-add-btn, [onclick]');
    if (target && (!e.relatedTarget || !target.contains(e.relatedTarget))) ring.classList.add('hovered');
  }, { passive: true });
  document.addEventListener('mouseout', e => {
    const target = e.target.closest('a, button, .product-add-btn, [onclick]');
    if (target && (!e.relatedTarget || !target.contains(e.relatedTarget))) ring.classList.remove('hovered');
  }, { passive: true });
