  ...
  function animateCursor() {
    ...
    // Dirty check: only update DOM if interpolated positions changed significantly (> 0.1px)
    if (Math.abs(dx - rDx) > 0.1 || Math.abs(dy - rDy) > 0.1 ||
        Math.abs(rx - rRx) > 0.1 || Math.abs(ry - rRy) > 0.1) {
  ...
  // Optimized event delegation for cursor hover states
