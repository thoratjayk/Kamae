  ...
  // Bolt: Only update coordinates on mousemove to keep the listener light
  document.addEventListener('mousemove', e => { mx = e.clientX; my = e.clientY; }, { passive: true });

  function animateCursor() {
    // Bolt: Smooth interpolation for the ring
    rx += (mx - rx) * 0.12;
    ry += (my - ry) * 0.12;

    // Bolt: Consolidate all cursor updates into requestAnimationFrame for sync with refresh rate.
  ...
  // Bolt: Use event delegation for cursor hover state.
  // This is O(1) memory vs O(N) and works for dynamically added content like products.
  ...
  document.addEventListener('mouseout', e => {
    const target = e.target.closest('a, button, [onclick], .product-card');
    const related = e.relatedTarget ? e.relatedTarget.closest('a, button, [onclick], .product-card') : null;
