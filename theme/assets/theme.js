  ...
  function animateCursor() {
    // Smoothing for dot (faster) and ring (slower)
    dx += (mx - dx) * 0.35;
    dy += (my - dy) * 0.35;
    rx += (mx - rx) * 0.12;
    ry += (my - ry) * 0.12;

    // Performance: Dirty check to avoid redundant DOM writes (0.1px threshold)
  ...
  // Efficient event delegation for cursor hover states
  document.addEventListener('mouseover', e => {
    if (e.target.closest('a, button, [onclick]')) ring.classList.add('hovered');
  }, { passive: true });
  document.addEventListener('mouseout', e => {
    if (e.target.closest('a, button, [onclick]') && !e.relatedTarget?.closest('a, button, [onclick]')) {
