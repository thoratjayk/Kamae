  ...
  // Use a single rAF loop for all cursor updates to consolidate DOM writes and use translate3d for GPU acceleration.
  document.addEventListener('mousemove', e => { mx = e.clientX; my = e.clientY; }, { passive: true });
  let lastDx = 0, lastDy = 0, lastRx = 0, lastRy = 0;
  function animateCursor() {
    // Dot follows mouse with slight lag for smoothness, or use 1 for instant.
    dx += (mx - dx) * 0.35;
    dy += (my - dy) * 0.35;
    ...
    // Dirty check: only update styles if movement is > 0.1px to reduce layout work when idle.
  ...
  // Use event delegation for hover states to handle dynamic content efficiently.
