  ...
  // Use passive listener and only store coordinates to reduce main thread blocking
  ...
  function animateCursor() {
    ...
    // Consolidate updates into a single rAF loop and use translate3d for GPU acceleration
  ...
  // Use event delegation for better performance and support for dynamic elements
  document.addEventListener('mouseover', e => {
    const target = e.target;
    // Specifically ignore cursor elements to prevent them from interfering with delegation
    if (target.id === 'cursorDot' || target.id === 'cursorRing') return;

    if (target.closest('a, button, .product-add-btn, [onclick]')) {
  ...
  document.addEventListener('mouseout', e => {
    ...
    // Specifically ignore cursor elements to prevent them from interfering with delegation
    if (related && (related.id === 'cursorDot' || related.id === 'cursorRing')) return;

    if (!related || !related.closest('a, button, .product-add-btn, [onclick]')) {
  ...
  const total = totalInCents / 100;
    ...
    // render items using DocumentFragment to batch DOM updates
  ...
  if (typeof window.shopifyLoadProducts === 'function') {
    window.shopifyLoadProducts().then(products => {
      ...
      // Use DocumentFragment to batch product card insertions
      const fragment = document.createDocumentFragment();
