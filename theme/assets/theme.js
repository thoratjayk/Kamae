  ...
  function animateCursor() {
    ...
    // Dot moves faster (0.35), Ring moves slower (0.12)
    dx += (mx - dx) * 0.35;
    dy += (my - dy) * 0.35;
    rx += (mx - rx) * 0.12;
    ry += (my - ry) * 0.12;

    // ⚡ Bolt: Dirty-check threshold (0.1px) avoids redundant DOM writes when stationary
  ...
  // ⚡ Bolt: Use event delegation for better performance and memory efficiency
  ...
  document.addEventListener('mouseout', e => {
    const target = e.target.closest('a, button, .product-add-btn, [onclick]');
    // ⚡ Bolt: Robust check for relatedTarget to prevent flickering
    if (target && !e.relatedTarget?.closest?.('a, button, .product-add-btn, [onclick]')) {
