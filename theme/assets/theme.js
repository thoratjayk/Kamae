  ...
  function animateCursor() {
    // ⚡ Bolt: Interpolation (lerp) for smooth movement
    dx += (mx - dx) * 0.35;
    dy += (my - dy) * 0.35;
    rx += (mx - rx) * 0.12;
    ry += (my - ry) * 0.12;

    // ⚡ Bolt: Dirty-check to skip DOM updates when cursor is stationary (0.1px threshold)
    if (Math.abs(mx - lastMx) > 0.1 || Math.abs(my - lastMy) > 0.1 || Math.abs(mx - rx) > 0.1 || Math.abs(my - ry) > 0.1) {
  ...
  // ⚡ Bolt: Event delegation for hover states (handles dynamic elements and reduces listener count)
