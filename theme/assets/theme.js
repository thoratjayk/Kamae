  ...
  let dx = 0, dy = 0; // Current dot positions

  // Passive listener for better scroll/touch performance
  ...
  function animateCursor() {
    ...
    // Smooth follow for dot (slight easing for fluid feel)
    dx += (mx - dx) * 0.45;
    dy += (my - dy) * 0.45;

    // "Dirty check" to avoid redundant DOM updates if movement is sub-pixel
  ...
  // Event delegation for cursor hover states (more efficient, supports dynamic content)
  ...
  const total = totalInCents / 100;
    ...
    // render items (batching DOM updates with DocumentFragment)
