  ...
  // Passive listener for better scroll performance and main-thread responsiveness
  document.addEventListener('mousemove', e => {
    mx = e.clientX;
    my = e.clientY;
  }, { passive: true });

  // Consolidated rAF loop for smooth, synced animation with zero layout thrashing
  function animateCursor() {
    ...
    // Dirty check: Only update DOM if movement exceeds 0.1px to save CPU/battery
    if (Math.abs(ddx) > 0.1 || Math.abs(ddy) > 0.1 || Math.abs(drx) > 0.1 || Math.abs(dry) > 0.1) {
      dx += ddx * 0.35; // Fast smoothing for dot
      dy += ddy * 0.35;
      rx += drx * 0.12; // Slower smoothing for ring
      ry += dry * 0.12;
  ...
  // Efficient event delegation: single listener for all interactive elements
  // This also handles dynamically added products without re-binding
  document.addEventListener('mouseover', e => {
    if (e.target.closest('a, button, .product-add-btn, [onclick]')) ring.classList.add('hovered');
  }, { passive: true });
