  ...
  // Optimized mousemove with passive listener
  document.addEventListener('mousemove', e => {
    mx = e.clientX;
    my = e.clientY;
  }, { passive: true });

  // Consolidated animation loop with dirty-check and translate3d
  function animateCursor() {
    // Only update DOM if mouse moved or ring/dot still interpolating
    const dotDist = Math.hypot(mx - dx, my - dy);
    const ringDist = Math.hypot(mx - rx, my - ry);

    if (dotDist > 0.1 || ringDist > 0.1 || mx !== lastMx || my !== lastMy) {
