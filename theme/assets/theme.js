  ...
  let dx = 0, dy = 0; // Current dot position for interpolation/dirty check

  // Optimize: Passive listener and only update coordinates
  ...
  function animateCursor() {
    ...
    // Performance: Dirty check to avoid redundant DOM updates (0.1px threshold)
    const ringDist = Math.sqrt(Math.pow(mx - rx, 2) + Math.pow(my - ry, 2));
    const dotDist = Math.sqrt(Math.pow(mx - dx, 2) + Math.pow(my - dy, 2));
  ...
  // Performance: Event delegation instead of individual listeners
