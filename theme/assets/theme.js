  ...
  // Use passive listener for better main-thread responsiveness
  document.addEventListener('mousemove', e => { mx = e.clientX; my = e.clientY; }, { passive: true });

  function animateCursor() {
    // Consolidate updates into a single RAF loop with smooth interpolation
    dx += (mx - dx) * 0.35;
    dy += (my - dy) * 0.35;
    rx += (mx - rx) * 0.12;
    ry += (my - ry) * 0.12;

    // Performance: Dirty check to avoid redundant DOM updates when stationary
  ...
  // Efficient event delegation for interactive elements (supports dynamic content)
