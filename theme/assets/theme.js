  ...
  // Optimized mouse move listener with passive option
  document.addEventListener('mousemove', e => {
    mx = e.clientX;
    my = e.clientY;
  }, { passive: true });

  // Consolidated animation loop for both dot and ring.
  // Uses translate3d to trigger GPU acceleration and minimizes layout thrashing.
  ...
  // Efficient event delegation for cursor hover effects.
  // This reduces memory usage and works for dynamically added elements (e.g. products from API).
  ...
  document.addEventListener('mouseout', e => {
    const target = e.target.closest('a, button, [onclick]');
    if (target) {
      // Avoid flicker when moving between a parent and its child
