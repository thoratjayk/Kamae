  ...
  // Use passive: true for mousemove for better performance
  document.addEventListener('mousemove', e => { mx = e.clientX; my = e.clientY; }, { passive: true });
  function animateCursor() {
    const ddx = mx - dx; const ddy = my - dy;
    const drx = mx - rx; const dry = my - ry;
    // Consolidation and dirty check (0.1px threshold) to avoid redundant DOM updates
    if (Math.abs(ddx) > 0.1 || Math.abs(ddy) > 0.1 || Math.abs(drx) > 0.1 || Math.abs(dry) > 0.1) {
      dx += ddx * 0.35; dy += ddy * 0.35;
      rx += drx * 0.12; ry += dry * 0.12;
  ...
  animateCursor();
  // Use event delegation for hover states to improve performance and support dynamic elements
