  ...
  function animateCursor() {
    ...
    // ⚡ Bolt: Dirty-check threshold (0.1px) to avoid redundant DOM updates
    if (Math.abs(dx - ldx) > 0.1 || Math.abs(dy - ldy) > 0.1) {
  ...
  // ⚡ Bolt: Use event delegation for better performance and support for dynamic elements
  document.addEventListener('mouseover', e => {
    const t = e.target.closest('a, button, .product-add-btn, [onclick]');
    if (t && (!e.relatedTarget || !t.contains(e.relatedTarget))) ring.classList.add('hovered');
  }, { passive: true });
  document.addEventListener('mouseout', e => {
    const t = e.target.closest('a, button, .product-add-btn, [onclick]');
    if (t && (!e.relatedTarget || !t.contains(e.relatedTarget))) ring.classList.remove('hovered');
  }, { passive: true });
